...
const DrawCanvas=({targetWord,onResult}:{targetWord:string;onResult:(r:string)=>void})=>{
  ...
  useEffect(()=>{const c=canvasRef.current;if(!c)return;drawRulers(c.getContext("2d")!);},[]);
  ...
  const clearCanvas=()=>{const c=canvasRef.current!;const ctx=c.getContext("2d")!;ctx.clearRect(0,0,c.width,c.height);drawRulers(ctx);setChecked(false);setAiResult(null);};
...
const SarfWriteTable=({word,onDone}:{word:Verb;onDone:()=>void})=>{
  ...
  const correct=checked?
    PRONOUNS.reduce((acc,_,i)=>acc+(isOk(`past_${i}`,word.conjugation.past[i])?1:0)+(isOk(`present_${i}`,word.conjugation.present[i])?1:0),0)
    +AMR_IDX.reduce((acc,i)=>acc+(isOk(`amr_${i}`,amrForm(i))?1:0),0):0;
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,flexWrap:"wrap",gap:8}}>
        ...
        <table style={{borderCollapse:"collapse",fontSize:12,minWidth:360}}>
          ...
          <tbody>{PRONOUNS.map((pr,i)=>{
            ...
            const mkCell=(k:string,target:string)=>{
              const ans=answers[k]||"";const ok=checked?isOk(k,target):null;
